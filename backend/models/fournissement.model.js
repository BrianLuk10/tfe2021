...
Fournissement.create = (newFournissement, result) => {
    sql.query("INSERT INTO fournissements SET ?", newFournissement, (err, res) => {
        if (err) {
        ...
        console.log("created fournissement: ", { id: res.insertId, ...newFournissement });
        result(null, { id: res.insertId, ...newFournissement });
    });
};

Fournissement.findById = (id_fournissement, result) => {
    sql.query(`SELECT * FROM Fournissements WHERE id_fournissement = ${id_fournissement}`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
...
Fournissement.findByIdProduit = (id_produits, result) => {
    sql.query(`SELECT id_fournissements, sum(stock_produits) as stock_produits, date_fournissement, exp_date FROM fournissements where id_produits = "${id_produits}"`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
...
Fournissement.updateById = (id, Fournissement, result) => {
    sql.query(
        `UPDATE Fournissements SET id_produits = ?, stock_produits = ?, date_fournissement = ?, exp_date = ? WHERE id_fournissement = ${id}`,
        [Fournissement.id_produits, Fournissement.stock_produits, Fournissement.date_fournissement, Fournissement.exp_date, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
...
Fournissement.remove = (id, result) => {
    sql.query("DELETE FROM fournissements WHERE id_fournisement = ?", id, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(null, err);
            return;
